...
public abstract class DailyEmployeeScoreAnalyticsReportMapper {

    @Mapping(target = "id", ignore = true)
    @Mapping(target = "entityId", source = "reportId")
    @Mapping(target = "type", source = "type")
    @Mapping(target = "content", source = "payload", qualifiedByName = "contentToEntity")
    public abstract SystemEvent toEntity(String reportId, SystemEventType type, DailyEmployeeScoreAnalyticsReport payload);

    @Named("contentToEntity")
    protected String generateContentToEntity(DailyEmployeeScoreAnalyticsReport payload) {
        return CsvUtil.getCsvFromEfficiencyByEmployees(payload.getEfficiencyByUsers());
    }
